#!/usr/bin/env python3
"""
Visual regression test for split zoom hiding the non-zoomed pane cleanly.

Scenario:
  - Render deterministic content in a single terminal pane and capture a baseline screenshot.
  - Split right and render distinct content into the right pane.
  - Zoom the left pane.
...
def main() -> int:
    with cmux(SOCKET_PATH) as client:
        ...
        try:
            ...
            time.sleep(0.6)
            before_screenshot = _capture_screenshot(client, "split_zoom_before")

            right_panel_id = client.new_split("right")
            time.sleep(0.5)
            ...
            left_panel_id = left_row["panelId"]
            right_panel_id = right_row["panelId"]

            _paint_terminal(client, right_panel_id, "CMUX ZOOM RIGHT CLIP PROBE")
            time.sleep(0.6)
            during_screenshot = _capture_screenshot(client, "split_zoom_during")

            client.focus_pane(left_pane_id)
            time.sleep(0.2)
            client.simulate_shortcut("cmd+shift+enter")
            time.sleep(1.0)
            after_screenshot = _capture_screenshot(client, "split_zoom_after")
            after_layout = client.layout_debug()

            before_crop = _crop_right_focus_region(before_screenshot, "split_zoom_before_crop")
            during_crop = _crop_right_focus_region(during_screenshot, "split_zoom_during_crop")
            after_crop = _crop_right_focus_region(after_screenshot, "split_zoom_after_crop")
            ...
            print("PASS: zoomed pane hides the clipped sibling visually")
